...
- `e26d7fa` fix(activitypub): bind keys to owner host (adds `TestVerifyRejectsCrossHostKeyOwner`) — my [High] ActivityPub signature item.
- `5fb7371` fix(indieauth): synchronize pending client requests — my [High] IndieAuth concurrency item.
- `4f5cf9c` fix(chat): synchronize client and stream state maps, plus follow-ups `a19b113` and `c2c1190` — my [High] chat/stream concurrency item.
- `a8a2958` fix(indieauth): consume auth requests once — my [Medium] replay item.
- `e706360` fix(chat): enforce reserved username matching — my [Medium] blocklist item.
- `a3247cd` fix(webhooks): handle invalid destinations safely — my [Low] webhook item.
- `6efb0f4` fix(fediverse): reject malformed handles safely — my [Low] webfinger/remoteFollow item.
